        ...
        <Item
          ...
          works={[
            [
              ...
              "<strong>기술적 의사결정 및 비즈니스 서포트</strong><br/> - 글로벌 진출 기술적 검토 및 리딩: 기술적 리서치, 서비스 현재 상태 진단 및 아키텍처 설계, 실무자 커뮤니케이션 등을 진행<br/>- 데이터베이스 형상 관리를 위한 Flyway 도입 결정 및 위임: 분산된 서버 환경에서 플랫폼에 독립적인 마이그레이션 도구 도입을 결정하고, 백엔드 개발자 분이 주도적으로 리딩하고 완수할 수 있도록 서포트<br/>- 개인형 AI 에이전트 정부 R&D 과제 기획 및 기술 설계: 스케일업 팁스 '분산 인프라 기반 개인형 AI 에이전트' 과제를 위한 기술 아키텍처 설계 및 연구개발계획서 기술 부문 작성",
              "<strong>AI 기술 도입 및 개발생산성 향상</strong><br/>- AI 기반 코드 리뷰 프로세스 정착: AI 코드 리뷰 도구(CodeRabbit)를 도입해 PR 퀄리티 향상 및 리뷰 부담 완화<br/>- AI 엔지니어링 환경 구축: Claude 도입을 위한 공통 하네스 구축 및 온보딩 가이드를 배포하여 AI 활용역량 평준화<br/>- QA 효율화 및 고도화를 위한 테스트 자동화 추진: Claude 도입을 통해 자동화 구축 견인, FE/BE 배포 시 QA 테스트가 자동으로 트리거 되도록 파이프라인 연동 지원",
              "<strong>테크 리더십 및 아키텍처 전환</strong><br/>- 데이터베이스 메이저 업그레이드: Postgres 13 EoL 대응을 위해 Blue/Green 배포 방식을 도입하여, Postgres 16 메이저 업그레이드 수행<br/>- 전사 데이터 분석 인프라 구축: 클라우드 기반 분석 환경 조성을 위해 AWS-GCP 간 Site-to-Site VPN -> GCP Datastream -> BigQuery 로 이어지는 ELT 파이프라인 설계 및 구축<br/>- 인프라 IaC 도입 및 개발환경 개선: Terraform 및 Terragrunt를 도입하여 코드 기반 인프라 관리 체계 구축, AwS ECS 기반의 Preview 환경을 설계, 구축하여 개발 검증 효율성 극대화<br/>- 아키텍처 전환 주도: 기존 레거시 시스템의 확장성 및 고도화를 위해 Kotlin Spring MSA 구조 전환 리딩<br/>- SaaS 및 인프라 자산 관리: AWS, GCP, Azure, Datadog, Sentry, Claude 등 개발팀 소프트웨어 구독 및 인프라 계약/비용 관리 최적화",
              "<strong>기술 부채 해소 및 성능 최적화</strong><br/>- 서비스 아키텍처 구조 리빌딩: 프론트엔드에 하드코딩되어 있던 많은 상품 데이터를 서버 레이어로 이관하고, 상품 관련 API를 전면 재설계해서 도메인 위계를 잡고, 동적 상품 판매 구조로 체질 개선<br/> - 어플리케이션 성능 최적화: 수초이상 걸리던 API의 N+1 쿼리 개선을 통해 약 99% 성능 개선<br/>- 레거시 문서화 정비: 클로드를 적극 활용해 수백 개에 달하는 API Swagger 및 Postman 컬렉션 자동 정비 및 최신화<br/>- CI/CD 배포 인프라 튜닝: Health check, Connection Draining, Green 대기 시간 최적화를 통해 배포 속도 및 안정성 향상<br/>- 클라우드 비용 최적화: 미사용 인프라 삭제, ECS 태스크 수 최적화를 통해 인프라 비용 절감<br/>- 웹 성능 및 네트워크 프로토콜 최적화: Cloudfront HTTP/3 프로토콜 도입하여 엣지 로케이션 연결 효율성 개선 및 레이턴시 단축<br/>- CI/CD 인프라 리소스 최적화 및 보안강화: FE 모노레포(Turborepo) 빌드 과정에서 발생하는 메모리 고갈(OOM) 문제를 Github Large Runner 도입으로 해결하고, 고정 IP를 통해 개발 환경에 대한 접근통제 및 보안성 확보",
